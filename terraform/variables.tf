...
variable "portal_public_key" {
  type        = string
  description = "SSH public key content for the portal instance (contents of secrets/portal-lab-ssh-key.pub, generated by scripts/03-terraform-apply.sh). Passed as a variable rather than read via file() so 'terraform validate' doesn't need the gitignored key material to exist."
}
